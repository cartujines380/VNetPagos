...
{
    public class InstitutionalContentController : BaseController
    {
        private readonly IPageClientService _pageClientService;

        public InstitutionalContentController(IPageClientService pageClientService)
        {
            _pageClientService = pageClientService;
        }

        [CustomAuthentication(Actions.InstitutionalContentDetails)]
        public async Task<ActionResult> Details()
        {
            var pages = await _pageClientService.FindAll();

            var page = pages.First(p => p.PageType == PageTypeDto.InstitutionalContent);
            if(page.Content != null) page.Content = page.Content.Replace(Environment.NewLine, string.Empty);

            return View(page.ToModel());
        }

        [CustomAuthentication(Actions.InstitutionalContentEdit)]
