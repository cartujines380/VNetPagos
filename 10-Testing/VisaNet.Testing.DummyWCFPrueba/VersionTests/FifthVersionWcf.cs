...
//using ServiceReference = VisaNet.Testing.DummyWCFPrueba.Demouy_Production_v05;
//using ServiceReference = VisaNet.Testing.DummyWCFPrueba.Azure_VisaNet_v030006;
//using ServiceReference = VisaNet.Testing.DummyWCFPrueba.Demouy_Development_v05;
using ServiceReference = VisaNet.Testing.DummyWCFPrueba.Demouy_Testing_v05;
//using ServiceReference = VisaNet.Testing.DummyWCFPrueba.Demouy_Development_v05;

namespace VisaNet.Testing.DummyWCFPrueba.VersionTests
{
    public static class FifthVersionWcf
    {
        private static CultureInfo _culture;
        private const string Folder = "FifthVersionData";
        ...
        private static void ConsoleOptions()
        {
            ...
            Console.WriteLine("ur   => Listado de llamadas a url_transaccion");
        ...
        private static void GetUrlTransaccion()
        {
            var cliente = new ServiceReference.VNPAccessClient();

            var fileName = ConfigurationManager.AppSettings["ConsultaUrlTransaction"];
            var path = Path.Combine(Directory.GetCurrentDirectory(), "Data", Folder, fileName);
            NLogLogger.LogEvent(NLogType.Info, path);
            ...
            {
                string json = r.ReadToEnd();
                var info = JsonConvert.DeserializeObject<IEnumerable<ConsultaUrlTransaccionModel>>(json, new JsonSerializerSettings()
                {
                    Culture = _culture
                });
                ...
                foreach (var consulta in info)
                {
                    var data = new ServiceReference.ConsultaUrlTransaccionData()
                    {
                        IdApp = consulta.IdApp,
                    ...
                    var result = cliente.ConsultaUrlTransaccion(data);
                    Console.WriteLine(result.CodResultado);
                    if (result.ListadoLlamadasUrlTransaccion != null && result.ListadoLlamadasUrlTransaccion.Any())
                    {
                        foreach (var urldata in result.ListadoLlamadasUrlTransaccion)
                        {
                            Console.WriteLine("UrlTransaccion IdOperacion: " + urldata.IdOperacion + ", HttpCodigoRespuesta: " + urldata.HttpCodigoRespuesta);
                        }
                    }
                    else
                    {
