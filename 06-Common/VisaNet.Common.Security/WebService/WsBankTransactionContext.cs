...
{
    public class WsBankTransactionContext : IWsBankTransactionContext
    {
        ...
        const string EXTERNAL_USER_NAME = "EXTERNAL_USER_NAME";

        public WsBankTransactionContext()
        {
            TransactionIdentifier = Guid.NewGuid();
            TransactionDateTime = DateTime.Now;
            UserName = "WebServiceBank";

            if ((HttpContext.Current.Session[EXTERNAL_USER_NAME] != null))
            {
                UserName = (string)HttpContext.Current.Session[EXTERNAL_USER_NAME];
            }
