...
namespace Sucive
{
    public class Sucive15 : BaseSucive, ISucive15
    {

        private readonly ILoggerService _loggerService;

        public Sucive15(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private Result15GetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param)
        {
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new Result15GetConsultaDeudaCc { AuxiliarCobro = -4 };
                }
            }

            if (codigoretorno.Equals("000"))
            {
                return new Result15GetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }
            return null;
        }

        private BillSuciveDto GetConsultaDeuda(long auxiliarCobro, SA_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array)
        {
        ...
        public long ConfirmarPago(long numeroPreFactura, string sucursal, string transactionId)
        {
            ...
            NotificationFix(numeroPreFactura.ToString(),transactionId,codigoretorno,mensajeretorno,DepartamentDtoType.Salto, null);
        ...
        public BillSuciveDto CheckIfBillPayable(string lines, int idPadron, string param)
        {
            ...
            var list = new List<SA_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new SA_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
        ...
        public List<ConciliationSuciveDto> Conciliation(DateTime date)
        {
            ...
            foreach (var item in result)
            {
                var dto = new ConciliationSuciveDto()
                {
                    ...
                    Currency = "UYU",
                    Date = date,
                    Departament = DepartamentDtoType.Salto
                };
                list.Add(dto);
            }
            return list;
    ...
    public class Result15GetConsultaDeudaCc
    {
        public SA_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
