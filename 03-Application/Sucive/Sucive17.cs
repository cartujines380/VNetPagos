...
namespace Sucive
{
    public class Sucive17 : BaseSucive, ISucive17
    {

        private readonly ILoggerService _loggerService;

        public Sucive17(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private Result17GetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param)
        {
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new Result17GetConsultaDeudaCc { AuxiliarCobro = -4 };
                }
            }

            if (codigoretorno.Equals("000"))
            {
                return new Result17GetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }
            return null;
        }

        private BillSuciveDto GetConsultaDeuda(long auxiliarCobro, SO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array)
        {
        ...
        public long ConfirmarPago(long numeroPreFactura, string sucursal, string transactionId)
        {
            ...
            NotificationFix(numeroPreFactura.ToString(),transactionId,codigoretorno,mensajeretorno,DepartamentDtoType.Soriano, null);
        ...
        public BillSuciveDto CheckIfBillPayable(string lines, int idPadron, string param)
        {
            ...
            var list = new List<SO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new SO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
        ...
        public List<ConciliationSuciveDto> Conciliation(DateTime date)
        {
            ...
            foreach (var item in result)
            {
                var dto = new ConciliationSuciveDto()
                {
                    ...
                    Currency = "UYU",
                    Date = date,
                    Departament = DepartamentDtoType.Soriano
                };
                list.Add(dto);
            }
            return list;
    ...
    public class Result17GetConsultaDeudaCc
    {
        public SO_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
