...
namespace Sucive
{
    public class Sucive4 : BaseSucive, ISucive4
    {

        private readonly ILoggerService _loggerService;

        public Sucive4(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private Result4GetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param)
        {
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new Result4GetConsultaDeudaCc { AuxiliarCobro = -4 };
                }
            }

            if (codigoretorno.Equals("000"))
            {
                return new Result4GetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }
            return null;
        }

        private BillSuciveDto GetConsultaDeuda(long auxiliarCobro, CO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array)
        {
        ...
        public long ConfirmarPago(long numeroPreFactura, string sucursal, string transactionId)
        {
            ...
            NotificationFix(numeroPreFactura.ToString(),transactionId,codigoretorno,mensajeretorno,DepartamentDtoType.Colonia, null);
        ...
        public BillSuciveDto CheckIfBillPayable(string lines, int idPadron, string param)
        {
            ...
            var list = new List<CO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new CO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
        ...
        public List<ConciliationSuciveDto> Conciliation(DateTime date)
        {
            ...
            foreach (var item in result)
            {
                var dto = new ConciliationSuciveDto()
                {
                    ...
                    Currency = "UYU",
                    Date = date,
                    Departament = DepartamentDtoType.Colonia
                };
                list.Add(dto);
            }
            return list;
    ...
    public class Result4GetConsultaDeudaCc
    {
        public CO_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
