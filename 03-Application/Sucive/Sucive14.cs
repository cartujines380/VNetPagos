...
namespace Sucive
{
    public class Sucive14 : BaseSucive, ISucive14
    {

        private readonly ILoggerService _loggerService;

        public Sucive14(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private Result14GetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param)
        {
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new Result14GetConsultaDeudaCc { AuxiliarCobro = -4 };
                }
            }

            if (codigoretorno.Equals("000"))
            {
                return new Result14GetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }
            return null;
        }

        private BillSuciveDto GetConsultaDeuda(long auxiliarCobro, RO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array)
        {
        ...
        public long ConfirmarPago(long numeroPreFactura, string sucursal, string transactionId)
        {
            ...
            NotificationFix(numeroPreFactura.ToString(),transactionId,codigoretorno,mensajeretorno,DepartamentDtoType.Rocha, null);
        ...
        public BillSuciveDto CheckIfBillPayable(string lines, int idPadron, string param)
        {
            ...
            var list = new List<RO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new RO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
    ...
    public class Result14GetConsultaDeudaCc
    {
        public RO_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
