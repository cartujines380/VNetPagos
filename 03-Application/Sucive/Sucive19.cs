...
namespace Sucive
{
    public class Sucive19 : BaseSucive, ISucive19
    {

        private readonly ILoggerService _loggerService;

        public Sucive19(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private Result19GetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param)
        {
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new Result19GetConsultaDeudaCc { AuxiliarCobro = -4 };
                }
            }

            if (codigoretorno.Equals("000"))
            {
                return new Result19GetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }
            return null;
        }

        private BillSuciveDto GetConsultaDeuda(long auxiliarCobro, TT_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array)
        {
        ...
        public long ConfirmarPago(long numeroPreFactura, string sucursal, string transactionId)
        {
            ...
            NotificationFix(numeroPreFactura.ToString(),transactionId,codigoretorno,mensajeretorno,DepartamentDtoType.Treinta_y_Tres, null);
        ...
        public BillSuciveDto CheckIfBillPayable(string lines, int idPadron, string param)
        {
            ...
            var list = new List<TT_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new TT_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
        ...
        public List<ConciliationSuciveDto> Conciliation(DateTime date)
        {
            ...
            foreach (var item in result)
            {
                var dto = new ConciliationSuciveDto()
                {
                    ...
                    Currency = "UYU",
                    Date = date,
                    Departament = DepartamentDtoType.Treinta_y_Tres
                };
                list.Add(dto);
            }
            return list;
    ...
    public class Result19GetConsultaDeudaCc
    {
        public TT_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
