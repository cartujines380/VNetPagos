...
namespace Sucive
{
    public class Sucive10 : BaseSucive, ISucive10
    {
        private readonly ILoggerService _loggerService;

        public Sucive10(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private Result10GetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param)
        {
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new Result10GetConsultaDeudaCc { AuxiliarCobro = -4 };
                }
            }

            if (codigoretorno.Equals("000"))
            {
                return new Result10GetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }
            return null;
        }

        private BillSuciveDto GetConsultaDeuda(long auxiliarCobro, MO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array)
        {
        ...
        public long ConfirmarPago(long numeroPreFactura, string sucursal, string transactionId)
        {
            ...
            string codigoretorno = "000";
            string mensajeretorno = "000";

            ServicePointManager.Expect100Continue = false;
            ServicePointManager.MaxServicePointIdleTime = 5000;

            var result = suciveservice.Execute(numeroPreFactura, sucursal, out codigoretorno, out mensajeretorno); // long.Parse(DateTime.Today.ToString("yyyyMMddhhmmss")); 
            NotificationFix(numeroPreFactura.ToString(),transactionId,codigoretorno,mensajeretorno,DepartamentDtoType.Montevideo, null);
        ...
        public void ReversoPagoB(long numeroFactura)
        {
            var suciveservice = new Ws15ReversobSoapPortClient();
            string mensajeretorno ;

            ServicePointManager.Expect100Continue = false;
            ServicePointManager.MaxServicePointIdleTime = 5000;
            var reverse = new SDT_ListaAnuSDT_ListaAnuItem()
                          {
                              TipoDato = "COB",
                              Dato = numeroFactura.ToString()
                          };
            NLogLogger.LogEvent(NLogType.Info, "Sucive Intento reverso. Numero de factura: " + numeroFactura);
            var result = suciveservice.Execute(new []{reverse},out mensajeretorno);

            NLogLogger.LogEvent(NLogType.Info, string.Format("Sucive Resultado de intento reverso. Numero de factura: {0}. result {1}, mensajeretorno {2}", numeroFactura,result,mensajeretorno));
        ...
        {
            //return new BillSuciveDto()
            //{
            //    Amount = 1350 * 6,
            //    Currency = "UYU",
            //    Line = "1;",
            //    Codigo = "200",
            //    Description = "Patente ANUAL",
            //    Year = "2017",
            //    IdPadron = 123,
            //    ExpirationDate = new DateTime(2017, 1, 20)
            //};

            var arrayLines = lines.Split(';');
            var result = GetConsultaDeudaCc(idPadron, param);

            var list = new List<MO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new MO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
        ...
        public List<ConciliationSuciveDto> Conciliation(DateTime date)
        {
            ...
            foreach (var item in result)
            {
                var dto = new ConciliationSuciveDto()
                {
                    ...
                    Currency = "UYU",
                    Date = date,
                    Departament = DepartamentDtoType.Montevideo
                };
                list.Add(dto);
            }
            return list;
        ...
        //public List<BillSuciveDto>  GenerateBIlls()
        //{
        //    return new List<BillSuciveDto>()
        //    {
        //        new BillSuciveDto(){Amount = 1350, Currency = "UYU", Line = "1;", Codigo = "200", Description = "Patente 1/6", Year = "2017", IdPadron = 123, ExpirationDate = new DateTime(2017,1,20)},
        //        new BillSuciveDto(){Amount = 1350, Currency = "UYU", Line = "2;", Codigo = "200", Description = "Patente 2/6", Year = "2017", IdPadron = 123, ExpirationDate = new DateTime(2017,3,20)},
        //        new BillSuciveDto(){Amount = 1350, Currency = "UYU", Line = "3;", Codigo = "200", Description = "Patente 3/6", Year = "2017", IdPadron = 123, ExpirationDate = new DateTime(2017,5,20)},
        //        new BillSuciveDto(){Amount = 1350, Currency = "UYU", Line = "4;", Codigo = "200", Description = "Patente 4/6", Year = "2017", IdPadron = 123, ExpirationDate = new DateTime(2017,9,20)},
        //        new BillSuciveDto(){Amount = 1350, Currency = "UYU", Line = "5;", Codigo = "200", Description = "Patente 5/6", Year = "2017", IdPadron = 123, ExpirationDate = new DateTime(2017,7,20)},
        //        new BillSuciveDto(){Amount = 1350, Currency = "UYU", Line = "6;", Codigo = "200", Description = "Patente 6/6", Year = "2017", IdPadron = 123, ExpirationDate = new DateTime(2017,11,20)}
        //    };
        //}
    }


    public class Result10GetConsultaDeudaCc
    {
        public MO_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
