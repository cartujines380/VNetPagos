...
namespace Sucive
{
    public class Sucive2 : BaseSucive, ISucive2
    {

        private readonly ILoggerService _loggerService;

        public Sucive2(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private Result2GetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param)
        {
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new Result2GetConsultaDeudaCc { AuxiliarCobro = -4 };
                }
            }

            if (codigoretorno.Equals("000"))
            {
                return new Result2GetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }
            return null;
        }

        private BillSuciveDto GetConsultaDeuda(long auxiliarCobro, CA_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array)
        {
        ...
        public long ConfirmarPago(long numeroPreFactura, string sucursal, string transactionId)
        {
            ...
            NotificationFix(numeroPreFactura.ToString(),transactionId,codigoretorno,mensajeretorno,DepartamentDtoType.Canelones, null);
        ...
        public BillSuciveDto CheckIfBillPayable(string lines, int idPadron, string param)
        {
            ...
            var list = new List<CA_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new CA_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
    ...
    public class Result2GetConsultaDeudaCc
    {
        public CA_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
