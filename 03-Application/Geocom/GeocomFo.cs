...
{
    //FLORIDA
    public class GeocomFo : BaseGeocom, IGeocomFo
    {
        
        private readonly ILoggerService _loggerService;

        public GeocomFo(ILoggerService loggerService, IServiceFixedNotification serviceFixedNotification)
        ...
        private int BusquedaCc(string[] refs, string param, string type)
        {
            ...
            NotificationFix(number, mensajeretorno, -1, refs, string.Empty, param, "Ws02BusquedaCCSoapPortClient", DepartamentDtoType.Florida, string.Empty);

            if (result == null)
            {
                NLogLogger.LogEvent(NLogType.Error, string.Format("Geocom BusquedaCc Florida. codigoretorno: {0}, mensajeretorno: {1}", codigoretorno, mensajeretorno));
                throw new ProviderFatalException(CodeExceptions.GEOCOM_NORESPONSE);
            }
            if (result.Count() > 1)
            {
                NLogLogger.LogEvent(NLogType.Error, "Geocom BusquedaCc Florida. Mas de un resultado devuelto");
                if (param.Equals("CIU"))
                {
        ...
        private ResultFoGetConsultaDeudaCc GetConsultaDeudaCc(int idPadron, string param, string cc)
        {
            ...
            if (result == null)
            {
                NLogLogger.LogEvent(NLogType.Error, string.Format("Geocom GetConsultaDeudaCc Florida. codigoretorno: {0}, mensajeretorno: {1}", codigoretorno, mensajeretorno));
                throw new ProviderFatalException(CodeExceptions.GEOCOM_NORESPONSE);
            }

            short number = 0;
            Int16.TryParse(codigoretorno, out number);
            NotificationFix(number, mensajeretorno, idPadron, null, string.Empty, param, "Ws03ConsultaDeudaCCSoapPortClient", DepartamentDtoType.Florida, string.Empty);
            ...
            if (number > 0)
            {
                ...
                if (number == 004)
                {
                    return new ResultFoGetConsultaDeudaCc { AuxiliarCobro = -4};    
                }
                if (number == 015)
                {
            ...
            if (number == 000)
            {
                return new ResultFoGetConsultaDeudaCc { AuxiliarCobro = auxiliarCobro, List = result };
            }

            throw new ProviderFatalException(CodeExceptions.GEOCOM_NORESPONSE);
        }

        private BillGeocomDto GetConsultaDeuda(int auxiliarCobro, GFO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] array, string param, int idPadron)
        {
            ...
            if (result == null)
            {
                NLogLogger.LogEvent(NLogType.Error, string.Format("Geocom GetConsultaDeuda Florida. codigoretorno: {0}, mensajeretorno: {1}", codigoretorno, mensajeretorno));
                throw new ProviderFatalException(CodeExceptions.GEOCOM_NORESPONSE);
            }
            ...
            NotificationFix(number, mensajeretorno, idPadron, null, string.Empty, param, "Ws04BConsultaDeudaSoapPortClient", DepartamentDtoType.Florida, string.Empty);
            ...
            NLogLogger.LogEvent(NLogType.Error, "Geocom GetConsultaDeuda Florida. Devulvo pre bill number -1");
        ...
        public long ConfirmarPago(int numeroPreFactura, string sucursal, string transactionId, string param)
        {
            ...
            short number = 0;
            Int16.TryParse(codigoretorno, out number);
            NotificationFix(number, mensajeretorno, -1, null, numeroPreFactura.ToString(), param, "Ws05ConfirmacionSoapPortClient", DepartamentDtoType.Florida,transactionId);
        ...
        public BillGeocomDto CheckIfBillPayable(string lines, int idPadron, string param)
        {
            ...
            var list = new List<GFO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem>();
            foreach (var item in result.List)
            {
                var cobranza = new GFO_04.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem()
                {
                    Anio = item.Anio,
        ...
        public List<ConciliationGeocomDto> Conciliation(DateTime date)
        {
            ...
            //                  Currency = "UYU",
            //                  Date = date,
            //                  Departament = DepartamentDtoType.Florida
            //              };
            //    list.Add(dto);
            //}
            return list;
    ...
    class ResultFoGetConsultaDeudaCc
    {
        public GFO_03.SDT_ColeccionCobranzaSDT_ColeccionCobranzaItem[] List { get; set; }
